...
public class Properties
{
    ...
    public GodotObject GodotObject { get; set; }
    ...
    /// <param name="pGodotObject">A Godot object containing the property / method</param>
    /// <param name="pReference">The name of the property / method</param>
    /// <param name="pIsMethod">Set to True if this is a method</param>
    public Properties(string pName, GodotObject pGodotObject, string pReference, bool pIsMethod)
    {
        Name = pName;
        GodotObject = pGodotObject;
...
///                     public override void _Process(double delta) { _xPos = Position.x; }
...
//*-------------------------------------------------------------------------*//
    ...
    public override void _Ready()
    {
        ...
        FontFile dynamic_font = new FontFile
        {
            FontName = "res://src/utils/debug_panel/Flexi_IBM_VGA_True.ttf",
            FixedSize = FontSize
    ...
    public override void _Process(double delta)
    {
        ...
        if (Drawcalls) _drawcalls.Text = "Drawcalls:" + Performance.GetMonitor(Performance.Monitor.RenderTotalDrawCallsInFrame).ToString();
...
//*-------------------------------------------------------------------------*//
    ...
    private void _Update_Properties()
    {
        ...
        foreach (Properties property in _propertiesList)
        {
            ...
            if (WeakRef(property.GodotObject)?.GetRef() != null)
            {
                // for a method, need to use "Call"
                if (property.IsMethod)
                    property_result = property.GodotObject.Call(property.Reference);
                else
                    property_result = property.GodotObject.Get(property.Reference);
