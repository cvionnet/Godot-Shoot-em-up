...
namespace BulletBallet.objects.items;
...
public partial class ItemsBrain : Node2D
{
    ...
    private List<objects.items.ItemGeneric> _listItems = new List<objects.items.ItemGeneric>();     // TODO : performance => use an ARRAY ?  (et ajouter l'id dans les propriétés du ItemGeneric)
    private List<Item> _listUniqueItems = new List<Item>();           // items that appear only once during the game
    private objects.items.ItemGeneric _itemTouched = new objects.items.ItemGeneric();
    ...
    private void Create_Item()
    {
            ...
            && _listItems.Count(i => i.ItemProperties.ActionName == random_item) < itemProperties.MaxVisibleInstance)
        {
            ...
            if (itemProperties.SpritePath != null)
            {
                ...
                objects.items.ItemGeneric instance = _spawnItems.Add_Instance<objects.items.ItemGeneric>(null, newPosition);
